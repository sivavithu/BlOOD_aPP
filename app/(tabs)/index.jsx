    ...
    const fetchDonationData = async () => {
      try {
        ...
        const { data: donationData, error: donationError } = await supabase
          ...
          .select('no_of_bottles,date') // Include donation date
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const { data: notifications, error } = await supabase
          .from('notifications')
          .select('*')
          .eq('user_id', session.user.id); // Assuming the notifications table has a `user_id` field
              ...
              <Pressable
                onPress={() => {
                  if (hasNotifications) {
                    ...
                    Alert.alert('No Notifications', 'There are no notifications available.');
                  }
                }}
              >
